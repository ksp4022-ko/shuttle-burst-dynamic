...
export const v8IntroSchema = z.object({
  ...
  timing: z.object({
    standoffFrames: z.number().int().min(1).max(60),
    anticipationFrames: z.number().int().min(1).max(60),
    attackFrames: z.number().int().min(1).max(60),
    impactFrames: z.number().int().min(1).max(30),
    knockbackFrames: z.number().int().min(1).max(60),
    settleFrames: z.number().int().min(1).max(60),
    heroRevealFrames: z.number().int().min(1).max(60),
  ...
  hero: z.object({
    ...
    date: z.string(),
    cta: z.string(),
    heroX: rangedNumber(20, 80),
    heroY: rangedNumber(20, 70),
    heroWidth: rangedNumber(50, 95),
    ctaOffsetY: rangedNumber(-80, 120),
  }),
